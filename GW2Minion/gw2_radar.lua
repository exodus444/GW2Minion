...
function gw2_radar.loadSettings()
	for name,radarType in pairs(gw2_radar.radarTypes) do
		for attribute, _ in pairs(radarType.variables) do
...
function gw2_radar.draw3DRadar()
	if (gw2_radar.radar3DActive) then
		...
		for _,entity in pairs(gw2_radar.trackEntities) do
			if (tvalid(entity) and entity.variables and entity.variables.radar3D.value) then
				local sPos = entity.sPos
				if (tvalid(sPos)) then
					local maxDistance =  6000
					local minDistance = 1000
					local entityDistance = entity.distance
					local distance = entityDistance < minDistance and minDistance or entityDistance > maxDistance and maxDistance or entityDistance
					...
					if (scale > 0.5) then
						...
						local text = sformat("[%s][%i]", entity.name, mround(entityDistance))
						local textSizeX, textSizeY = GUI:CalcTextSize(text)
						GUI:AddText(sPos.x - (textSizeX/2), sPos.y+textSizeY, 4294967295, text)

						local entityHealth = entity.health

						if (tvalid(entityHealth)) then
							local hpK = entityHealth.percent / 100
							GUI:AddRectFilled(sPos.x - (35 * scale), sPos.y + (32 * scale), sPos.x + (-35 + (70 * hpK)) * scale, sPos.y + (35 * scale), GUI:ColorConvertFloat4ToU32(math.abs(hpK - 1), hpK, 0, 1))
							GUI:AddRect(sPos.x - (37 * scale), sPos.y + (30 * scale), sPos.x + (37 * scale), sPos.y + (37 * scale), 4294967295)
							GUI:AddText(sPos.x + (40 * scale), sPos.y + (26 * scale), 4294967295, sformat('%i%', entityHealth.percent))
						end
...
function gw2_radar.drawCompass()
	if (gw2_radar.compassActive or gw2_radar.compassShowPath) then
		...
		if (gw2_radar.compassActive) then
			...
			for _,entity in pairs(gw2_radar.trackEntities) do
				if (tvalid(entity) and entity.variables and entity.variables.compass.value) then
					local rPos = entity.rPos
					if (tvalid(rPos) and rPos.x and rPos.y) then
						...
						if (mousePos.x > rPos.x - markerSize and mousePos.x < rPos.x + markerSize and mousePos.y > rPos.y - markerSize and mousePos.y < rPos.y + markerSize and not hovering) then
							...
							local text = entity.name .. "\n" .. mround(entity.distance) .. GetString(" away")
							local textSize = {}
...
function gw2_radar.drawCompassPath()
	if (gw2_radar.compassShowPath and tvalid(gw2_radar.compassPath)) then
...
function gw2_radar.parseCompassPath()
	...
	if (gw2_radar.compassShowPath) then
		...
		if (tvalid(path) and tvalid(lastPos)) then
			for id,pathPointPos in ipairs(path) do
				if (id >= ml_navigation.pathindex and tvalid(pathPointPos)) then
					local currPos = gw2_radar.worldToCompass(pathPointPos)
					if (tvalid(currPos)) then
...
function gw2_radar.updateFilterList()
	for _,radarType in pairs(gw2_radar.radarTypes) do
		if (tvalid(radarType) and (radarType.variables.compass.value or radarType.variables.radar3D.value)) then
			gw2_radar.filterList[radarType.list] = gw2_radar.filterList[radarType.list] or {}
			tinsert(gw2_radar.filterList[radarType.list],radarType)
		end
	end
end
...
function gw2_radar.parseEntities()
	if (gw2_radar.compassActive or gw2_radar.radar3DActive) then
		...
		for listName, radarTypes in pairs(gw2_radar.filterList) do
			local entityList = g[listName]("")

			-- update or remove
			for id, trackedEntity in pairs(gw2_radar.trackEntities) do
				local entity = entityList[id]
				if entity then
					trackedEntity.pos = entity.pos
					trackedEntity.health = entity.health
					trackedEntity.distance = entity.distance
					trackedEntity.variables	= radarType.variables
				else
					gw2_radar.trackEntities[id] = nil
				end
			end

			-- add missing
			for id, entity in pairs(entityList) do
				if (tvalid(entity)) then
					for _,radarType in pairs(radarTypes) do
						if (tvalid(radarType)) then
							if (gw2_radar.trackEntities[id] == nil and gw2_radar.matchFilterEntity(entity,radarType.filter)) then
								gw2_radar.trackEntities[id] = {
									id			= entity.id,
									name		= entity.name,
...
function gw2_radar.matchFilterEntity(entity, filterData)
	if (tvalid(entity) and tvalid(filterData)) then
		for _,filter in pairs(filterData) do
			if (tvalid(filter)) then
				for attribute, wantedValue in pairs(filter) do
					if (svalid(attribute) and (entity[attribute] == nil or entity[attribute] ~= wantedValue)) then
						return false
					end
				end
